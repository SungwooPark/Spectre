...
class newsBox(Frame):
    def __init__(self, master, text_color):
        ...
        #CREATE TEMP LABEL
        self.title = Label(self, font=('Helvetica',40), fg= text_color, bg="black",text='TEMP')
        self.ttle.pack(side = TOP, anchor = NE)
        #CREATE IMAGE LABEL
        pic = Image.open('NewsBox/choropleth_map.svg')
        self.choroMapPic = ImageTk.PhotoImage(pic)
        self.choroMap = Label(self, image = self.choroMapPic)
    ...
    def produce_map(search_term):
        ...
        self.choroMapPic = ImageTk.PhotoImage("NewsBox/choropleth_map.svg")
        self.choroMap.config(image = self.weatherImage)
